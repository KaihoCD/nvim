...
local function toggle_terminal(position, width, height)
	return function()
		width = width or G.size.float_width
		height = height or G.size.float_height
		local opts = {
			win = {
				position = position,
				width = width,
...
M.keys = {
	{
		'<c-\\>',
		toggle_terminal('bottom', 0, G.size.split_bottom),
		desc = 'Toggle Terminal',
		mode = { 'n', 't' },
	},
	{
		'|',
		toggle_terminal('right', G.size.split_right, 0),
		desc = 'Toggle Terminal',
		mode = { 'n', 't' },
	},
	{
